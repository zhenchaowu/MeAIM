...
class TopKEvaluator(object):
    ...
    def evaluate(self, batch_matrix_list, eval_data, is_test=False, idx=0):
        ...
        #print(len(pos_items))  #19445
        #a = [len(items) for items in pos_items]
        #print(np.sum(np.array(a)))
        #aaa
        pos_len_list = eval_data.get_eval_len_list()
        #print(len(pos_len_list)) #19445
        #print(np.sum(np.array(pos_len_list)))  #20559
        #aaa
        topk_index = torch.cat(batch_matrix_list, dim=0).cpu().numpy()
        
        #print('the size of topk_index is {}'.format(topk_index.shape))
    ...
    def _calculate_metrics(self, pos_len_list, topk_index):
        ...
        """
        #print(pos_len_list[5])
        #print(pos_len_list[17])
        #print(pos_len_list[24])
        #print('pos_len_list is {}'.format(len(pos_len_list)))   #19445
        #print('topk_index is {}'.format(topk_index.shape))   # (19445, 50)
        #aaa
        result_list = []
