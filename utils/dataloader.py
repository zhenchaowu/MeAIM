...
class TrainDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, shuffle=False):
        ...
        if config['use_neg_sampling']:  #True
            if self.use_full_sampling:  #False
                self.sample_func = self._get_full_uids_sample
            else:
                self.sample_func = self._get_neg_sample   #[3, 2048]
        else:
            self.sample_func = self._get_non_neg_sample  

        #self._get_history_items_u()
        self.neighborhood_loss_required = config['use_neighborhood_loss']  #False
        if self.neighborhood_loss_required:
            self.history_users_per_i = {}
            self._get_history_users_i()
            self.user_user_dict = self._get_my_neighbors(self.config['USER_ID_FIELD'])   #userID
            self.item_item_dict = self._get_my_neighbors(self.config['ITEM_ID_FIELD'])   #itemID
    ...
    def _get_neg_sample(self):
        cur_data = self.dataset[self.pr: self.pr + self.step]   #self.step: batch_size
        self.pr += self.step
        # to tensor
        ...
        #print('u_id is {}'.format(u_ids))  #length:2048
        
        # sampling negative items only in the dataset (train)
        neg_ids = self._sample_neg_ids(u_ids).to(self.device)
        
        #print('neg_ids is {}'.format(neg_ids.size()))   #size:2048
        #aaa
        
        # for neighborhood loss
        if self.neighborhood_loss_required: #False
        ...
        # merge negative samples
            ...
            batch_tensor = torch.cat((batch_tensor, neg_ids.unsqueeze(0)))
            #print('the size of batch_tensor is {}'.format(batch_tensor.size()))  #size:[3, 2048]
    ...
    def _sample_neg_ids(self, u_ids):
        #u_id: train_size:2048
        neg_ids = []
        for u in u_ids:
            # random 1 item
            iid = self._random()  #randomly select a item_id
            #print('iid is {}'.format(iid))
            #aaa
            while iid in self.history_items_per_u[u]:  #循环结构一直到选择出iid不在当前user交互的范围之内
                iid = self._random()
            neg_ids.append(iid)
...
class EvalDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, additional_dataset=None,
        ...
        self.eval_u = self.dataset.df[self.dataset.uid_field].unique()   #dataset中的userID
        # special for eval dataloader
        self.pos_items_per_u = self._get_pos_items_per_u(self.eval_u).to(self.device) #[user_id, item_id]
        self._get_eval_items_per_u(self.eval_u)  #包含每个user所交互的item长度
        # to device
    ...
    def _next_batch_data(self):
        ...
        #print(batch_mask_matrix.size())  #[2, 31860]
    ...
    def _get_pos_items_per_u(self, eval_users):
        ...
        #print(self.additional_dataset[0:10])
        #aaa
        
        uid_field = self.additional_dataset.uid_field  #train set中的userID
        iid_field = self.additional_dataset.iid_field  #train set中的itemID
        # load avail items for all uid
        uid_freq = self.additional_dataset.df.groupby(uid_field)[iid_field]
        #print('uid_freq is {}'.format(uid_freq))
        #aaa
        u_ids = []
        i_ids = []
        for i, u in enumerate(eval_users):
            u_ls = uid_freq.get_group(u).values  #u对应的itemID
            #print('the size of u_ls is {}'.format(u_ls))
            #aaa
            i_len = len(u_ls) 
    ...
    def _get_eval_items_per_u(self, eval_users):
        ...
        uid_freq = self.dataset.df.groupby(uid_field)[iid_field]
        for u in eval_users:
            u_ls = uid_freq.get_group(u).values  #当前user交互的item列表
            self.eval_len_list.append(len(u_ls))
            self.eval_items_per_u.append(u_ls)
        self.eval_len_list = np.asarray(self.eval_len_list)  #包含每个user所交互item的长度
