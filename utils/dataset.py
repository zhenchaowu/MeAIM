...
class RecDataset(object):
    def __init__(self, config, df=None):
        ...
        #print('the length of check_file_list is {}'.format(len(check_file_list)))   #length:1
        
        
        for i in check_file_list:
            file_path = os.path.join(self.dataset_path, i)
            #print('the file_path is {}'.format(file_path))
      
            if not os.path.isfile(file_path):
                raise ValueError('File {} not exist'.format(file_path))

        # load rating file from data path?
        self.load_inter_graph(config['inter_file_name'])  #baby_inter
    ...
    def split(self):
        ...
        for i in range(3):
            ...
            #temp_df.to_csv('bady_test_set.csv')
            #aaa
            dfs.append(temp_df)  #including train set, vailid set, test set.
